...
async def userAvatarDownLoad(url):
    fileName = dict_download_img(url, dirc="data/pictures/cache")
...
async def signPicMaker(ids, weather, nowTime, times, startTime):
    url=get_user_image_url(ids)
    ...
    draw.text((wi, int(bg.height * 0.25)), str(ids), (12, 0, 6), font=font)
    ...
    draw.text((wi, int(bg.height * 0.55)), "签到次数：" + str(times), (12, 0, 6), font=font)
...
def pic():
    ...
    try:
        ...
        with open("data/pictures/cache/" + ranpath + ".png", mode="wb") as f:
            f.write(r)  # 图片内容写入文件
        image_path = "data/pictures/cache/" + ranpath + ".png"
        layer = Image.open(image_path)
        layer = layer.resize((1920, 1080), resample=Image.LANCZOS)
        layer.save(image_path)
        return "data/pictures/cache/" + ranpath + ".png"
    except Exception as e:
        pass
        #logger.error(e)
        #logger.info("使用二号接口")
    ...
    try:
        ...
        image_path = "data/pictures/cache/9bFIzYz.png"
        return image_path
