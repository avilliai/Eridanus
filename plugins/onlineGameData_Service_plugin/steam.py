...
async def get_steam_game_description(game_id, proxy=None):
    ...
    async with httpx.AsyncClient(timeout=200, verify=False,
                                 proxies={"http://": proxy, "https://": proxy} if proxy else None) as client:
        response = await client.get(url, headers=headers)
        html = response.text
...
async def get_steam_game_search(keyword, proxy=None):
    ...
    async with httpx.AsyncClient(timeout=200,
                                 proxies={"http://": proxy, "https://": proxy} if proxy else None) as client:
        response = await client.get(url, headers=headers)
...
async def steam_query_game(keyword, proxy=None):
    # logger.info("选择代理：" + proxy)
    search_result = await get_steam_game_search(keyword, proxy)
    if search_result:
        img_url = search_result["avatar"]
        async with httpx.AsyncClient(timeout=200, proxies={"http://": proxy, "https://": proxy} if proxy else None,
