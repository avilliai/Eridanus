    ...
    for per_json_check in json_img:
        if isinstance(per_json_check, dict):  #判断是否为正确格式（字典）若是，则检查其中参数是否完整
            ...
            for per_json_check_per in per_json_check:
                if os.path.splitext(per_json_check_per)[1].lower() in [".jpg", ".png", ".jpeg",
                                                                       '.webp'] or per_json_check_per.startswith(
                        "http"):
            ...
            collect_img, collect_text = [], []
            if isinstance(per_json_check, str) and (
                    os.path.splitext(per_json_check)[1].lower() in [".jpg", ".png", ".jpeg",
                                                                    '.webp'] or per_json_check.startswith("http")):
