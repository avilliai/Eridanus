            ...
            if context_chara_check != '':
                if 'CV' in context_check:
                    ...
                    try:
                        ...
                        img_chara = "![" + img_list[
                            img_chara_count] + "](https://gal.manshuo.ink/usr/uploads/galgame/" + game_id + "/" + \
                                    img_list[img_chara_count] + ")"
                    except:
                        img_chara = ''
    ...
    for index, img in enumerate(images):
        try:
            ...
            if img.is_displayed():  # 检查图片是否可见
                ...
                for name_check in {'等级', '头像', '徽章', '光之友', '表情', 'Hikarinagi - 一个ACGN文化社区', 'DL',
                                   'steam'}:
                ...
                if is_loaded:
                    check = save_image_screenshot(img, index, img_url=img_url, img_name=img_name,
                                                  output_dir=game_id)  # 保存图片截图
                    if check:
    ...
    for index, img in enumerate(images):
        try:
            ...
            if img.is_displayed():  # 检查图片是否可见
                img_url = img.get_attribute("src")  # 获取图片的 src 属性（URL）
                img_name = img.get_attribute("alt")
                for name_check in {'等级', '头像', '徽章', '光之友', '表情', 'avatar', '有希日记', 'steam',
