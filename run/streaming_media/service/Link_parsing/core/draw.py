...
def creat_white_corners(canvas, content_width, content_height, padding_x, current_y, radius=None, type=None,
    ...
    draw = ImageDraw.Draw(mask)
    draw.rounded_rectangle([0, 0, shadow_blurred.size[0], shadow_blurred.size[1]], radius=radius, fill=0, outline=255,
                           width=2)
    ...
    for content in processed_contents:
        ...
        if isinstance(content, list) and isinstance(content[0], Image.Image):
            ...
            if len(content) == 1:  # 单张图片
                ...
                img = content[0]
                new_height = int(
                    ((content_width - (per_row_pic - 1) * padding) // per_row_pic) * img.height / img.width)
                total_height += new_height + padding
            elif len(content) == 4:
                img = content[0]
                ...
                if len(content) % per_row_pic == 0:
                    ...
                    len_number = len(content) // per_row_pic + 1
                img = content[0]
                new_height = int(((content_width - (
                            per_row_pic - 1) * padding) // per_row_pic) * img.height / img.width) * len_number
...
def handle_img(canvas, padding, padding_x, padding_x_text, avatar_path, font_size, name, Time, header_height,
               ...
               font_tx_title=None, avatar_json=None, per_row_pic=3):
    ...
    if layer is None or layer == 1:
        ...
        gradient_layer = create_gradient_background(
            (int(content_width + 2 * padding_x - 2 * padding), int(white_dy_img)),
            color1=(235, 239, 253), color2=(236, 255, 252))
        gradient_layer = add_rounded_corners(gradient_layer, radius=20)
        canvas = add_shaow_image_new(canvas, padding, content_width + 2 * padding_x - 2 * padding, total_height,
                                     padding, current_y - padding)
        creat_white_corners(canvas, int(content_width + 2 * padding_x - 2 * padding), int(white_dy_img), padding,
                            current_y - padding)
        ...
        canvas = creat_white_corners(canvas, content_width + padding_x, int(white_dy_img), padding_x,
                                     current_y - padding)
        gradient_layer = create_gradient_background((int(content_width + padding_x), int(white_dy_img)),
                                                    color1=(235, 239, 253), color2=(236, 255, 252))
        gradient_layer = add_rounded_corners(gradient_layer, radius=20)
        canvas = add_shaow_image_new(canvas, padding, content_width + padding_x, total_height, padding_x,
                                     current_y - padding)
    ...
    if type_software is not None:
        ...
        x_add = 10
        if layer is None or layer == 1:
            creat_white_corners(canvas, text_width + 2 * padding + 4, int(height_software), padding + x_add,
                                current_y - 70 - 4, radius=10)
            creat_white_corners(canvas, text_width + 2 * padding, int(height_software - 4), padding + 2 + x_add,
                                current_y - 70 - 2, radius=10, color=color_software)
            draw.text((padding * 2 + 2 + x_add, current_y - 70 - 2), f'{type_software}', fill=(255, 255, 255),
                      font=font_tx)
        elif layer == 2:
            creat_white_corners(canvas, text_width + 2 * padding + 4, int(height_software), padding_x + x_add,
                                current_y - 70 - 4, radius=10)
            creat_white_corners(canvas, text_width + 2 * padding, int(height_software - 4), padding_x + 2 + x_add,
                                current_y - 70 - 2, radius=10, color=color_software)
            draw.text((padding_x + padding + 2 + x_add, current_y - 70 - 2), f'{type_software}', fill=(255, 255, 255),
                      font=font_tx)
    ...
    if avatar_path:
        if layer is None or layer == 2:
            ...
            canvas = draw_text_step(canvas, position=(name_x, current_y - 30 + (avatar_size - font_size) // 2),
                                    text=name, font=font_tx_pil, text_color=(251, 114, 153))
            if Time is not None:
                canvas = draw_text_step(canvas, position=(name_x, current_y + 20 + (avatar_size - font_size) // 2),
                                        text=Time, font=font_tx, text_color=(148, 148, 148))
            if avatar_json is not None:
                ...
                if avatar_json['card_path']:
                    ...
                    canvas.paste(card, (int(padding_x_tx + padding_right + int(438 - width)), int(current_y - 15)),
                                 card)
                    if avatar_json['card_is_fan']:
                        canvas = draw_text_step(canvas,
                                                position=(int(padding_x_tx + padding_right + 140), int(current_y + 35)),
                                                text=avatar_json['card_number'], font=font_tx,
                                                text_color=avatar_json['card_color'])
            ...
            canvas.paste(avatar, (int(padding_x_tx), int(current_y)), avatar)
            name_x = padding_x_tx + avatar_size + padding_x_text - 20
            canvas = draw_text_step(canvas, position=(name_x, current_y - 10 + (avatar_size - font_size) // 2),
                                    text=name, font=font_tx_pil, text_color=(251, 114, 153))
            if Time is not None:
                text_width = font_tx_pil.getbbox(name)[2] - font_tx_pil.getbbox(name)[0]
                canvas = draw_text_step(canvas, position=(name_x + int(text_width) + 30,
                                                          current_y - 2 + (avatar_size - font_size) // 2), text=Time,
                                        font=font_tx, text_color=(148, 148, 148))
        ...
        if isinstance(content, list) and isinstance(content[0], Image.Image):
            ...
            if len(content) == 1:  # 单张图片
                ...
                if type_check is True and introduce is not None and introduce_height is not None:
                    creat_white_corners(canvas, content_width,
                                        int(content_width * img.height / img.width + introduce_height), padding_x_text,
                                        current_y)
                else:
                    creat_white_corners(canvas, content_width, int(content_width * img.height / img.width),
                                        padding_x_text, current_y)
                # 检查透明通道
                if img.mode == "RGBA":
            ...
            if type_check is True and introduce is not None:
                ...
                padding_x_check = padding_x_text
            # print(content)
            draw = ImageDraw.Draw(canvas)
            check_number = 0
            ...
            if len(content) == 1:
                ...
                if 'tag:' in line:
                    ...
                    canvas = draw_text_step(canvas, position=(padding_x_check, current_y), text=line, font=font_tx,
                                            text_color=(9, 132, 204), filepath=filepath, emoji_list=emoji_list)
                    ...
                    canvas = draw_text_step(canvas, position=(padding_x_check, current_y), text=line,
                                            font=font_tx_title, text_color=(0, 0, 0), filepath=filepath,
                                            emoji_list=emoji_list)
                    current_y_add = font_tx_title.getbbox("A")[3]
                else:
                    canvas = draw_text_step(canvas, position=(padding_x_check, current_y), text=line, font=font_tx,
                                            text_color=(0, 0, 0), filepath=filepath, emoji_list=emoji_list)
                    current_y_add = font_tx.getbbox("A")[3]
...
def draw_adaptive_graphic_and_textual(contents, canvas_width=1000, padding=25, font_size=30,
                                      ...
                                      avatar_json=None,
                                      color_software=None, orig_type_software=None, emoji_list=None,
                                      orig_emoji_list=None, per_row_pic=3):
    """
    图像绘制
    type类型说明：
    不传入则是默认从上往下的自适应排版
    ...
    # 文本自动换行
    (processed_contents, introduce_content, introduce_height, total_height) = handle_context(contents, font,
                                                                                             content_width,
                                                                                             total_height, padding,
                                                                                             type_check, introduce,
                                                                                             font_tx_introduce,
                                                                                             header_height,
                                                                                             type_software,
                                                                                             height_software,
                                                                                             ...
                                                                                             layer=layer,
                                                                                             font_tx_pil=font_tx_pil,
                                                                                             per_row_pic=per_row_pic)
    ...
    if type == 14:
        ...
        (orig_processed_contents, orig_introduce_content,
         orig_introduce_height, total_height) = handle_context(contents_dy, font, content_width - padding_x,
                                                               ...
                                                               avatar_path=avatar_path, layer=2,
                                                               font_tx_pil=font_tx_pil, per_row_pic=per_row_pic)
    ...
    canvas, current_y = handle_img(canvas, padding, padding_x, padding_x, avatar_path, font_size, name, Time,
                                   header_height,
                                   processed_contents, content_width, introduce, type_check, font_tx, font_tx_pil,
                                   introduce_height,
                                   introduce_content, introduce_content, font_tx_introduce, total_height=total_height,
                                   type_software=type_software,
                                   color_software=color_software, height_software=height_software,
                                   emoji_list=emoji_list, filepath=filepath, layer=layer
                                   , font_tx_title=font_tx_title, avatar_json=avatar_json, per_row_pic=per_row_pic)

    if type == 14:
        current_y_set = current_y
        type_check = True
        canvas, current_y = handle_img(canvas, padding, padding_x, padding_x + 20, orig_avatar_path, font_size,
                                       orig_name, orig_Time, header_height,
                                       orig_processed_contents, content_width - padding_x, introduce, type_check,
                                       font_tx, font_tx_pil,
                                       orig_introduce_height, orig_introduce_content, orig_introduce_content,
                                       font_tx_introduce, current_y,
                                       total_height, layer=2, type_software=orig_type_software,
                                       color_software=color_software, height_software=height_software,
                                       emoji_list=orig_emoji_list, filepath=filepath
                                       , font_tx_title=font_tx_title, avatar_json=avatar_json, per_row_pic=per_row_pic)
...
if __name__ == "__main__":
    ...
    draw_adaptive_graphic_and_textual(contents, name="Phigros", Time='"2025年01月03日 17:00"', type=14,
                                      avatar_path="E:\Others\github/bot\Eridanus\plugins/resource_search_plugin\Link_parsing\data\cache\orig_cover.jpg",
                                      contents_dy=contents_dy,
                                      orig_avatar_path="E:\Others\github/bot\Eridanus\plugins/resource_search_plugin\Link_parsing\data\cache\orig_cover.jpg",
                                      orig_name="漫朔", orig_Time='"2025年01月03日 17:00"', type_software='BiliBili',
