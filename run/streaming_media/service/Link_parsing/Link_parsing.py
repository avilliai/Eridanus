...
from run.streaming_media.service.Link_parsing.core.bangumi_core import claendar_bangumi_get_json, \
    bangumi_subject_post_json, bangumi_subjects_get_json_PIL
from run.streaming_media.service.Link_parsing.core.bili import bili_init, av_to_bv, download_b, info_search_bili
from run.streaming_media.service.Link_parsing.core.common import download_video, download_img, add_append_img, \
    GENERAL_REQ_LINK, get_file_size_mb
...
json_init = {'status': False, 'content': {}, 'reason': {}, 'pic_path': {}, 'url': {}, 'video_url': False,
             'soft_type': False}
...
name_qq_list = {'漫朔': 1270858640, '枫与岚': 2319804644, 'Lemony': 2424378897, 'forandsix': 1004704649,
                '魏咸哲': 1431631009}
...
async def bilibili(url, filepath=None, is_twice=None):
    ...
    if ('t.bilibili.com' in url or '/opus' in url or '/space' in url) and BILI_SESSDATA != '':
                ...
                if dynamic_info is not None:
                    ...
                    for module in dynamic_info['item']['modules']:
                        if 'module_author' in module:
                            ...
                            avatar_path = \
                            (await asyncio.gather(*[asyncio.create_task(download_img(owner_cover, f'{filepath}'))]))[0]
                            break
                    try:
                    ...
                    contents = await add_append_img(contents, await asyncio.gather(
                        *[asyncio.create_task(download_img(item['url'], f'{filepath}', len=len(pics_context))) for item
                          in pics_context]))
        ...
        if is_opus:
            ...
            if dynamic_info is not None:
                ...
                avatar_path = (await asyncio.gather(*[asyncio.create_task(download_img(owner_cover, f'{filepath}'))]))[
                    0]
                if orig_check == 1:
                    avatar_json = await info_search_bili(dynamic_info, is_opus, filepath=filepath,
                                                         card_url_list=card_url_list)
                    ...
                    if 'opus' in dynamic_info['item']['modules']['module_dynamic']['major']:
                        ...
                        if dynamic_info['item']['type'] == 'DYNAMIC_TYPE_ARTICLE':
                            ...
                            contents = await add_append_img(contents, await asyncio.gather(
                                *[asyncio.create_task(download_img(item, f'{filepath}', len=len(pics_context))) for item
                                  in pics_context]))
                        else:
                            contents.append(text_list_check)
                            for pic_check in opus_paragraphs['pics']:
                        ...
                        title, cover, pub_time = content['live_play_info']['title'], content['live_play_info']['cover'], \
                        content['live_play_info']['live_start_time']
                        contents.append(
                            (await asyncio.gather(*[asyncio.create_task(download_img(cover, f'{filepath}'))]))[0])
                        ...
                        title, desc, cover, bvid = paragraphs['title'], paragraphs['desc'], paragraphs['cover'], \
                        paragraphs['bvid']
                        contents.append(
                            (await asyncio.gather(*[asyncio.create_task(download_img(cover, f'{filepath}'))]))[0])
                        contents.append(title)
                        ...
                        if 'module_dynamic' in module:
                            if 'opus' in orig_context['modules']['module_dynamic']['major']:
                                ...
                                contents_dy = await add_append_img(contents_dy, await asyncio.gather(*[
                                    asyncio.create_task(
                                        download_img(item['url'], f'{filepath}', len=len(opus_orig_paragraphs['pics'])))
                                    for item in opus_orig_paragraphs['pics']]))
                            else:
                                orig_paragraphs = orig_context['modules']['module_dynamic']['major']['archive']
                                orig_title, orig_desc, orig_cover, orig_bvid = orig_paragraphs['title'], \
                                orig_paragraphs['desc'], orig_paragraphs['cover'], orig_paragraphs['bvid']
                                contents_dy.append((await asyncio.gather(
                                    *[asyncio.create_task(download_img(orig_cover, f'{filepath}'))]))[0])
                                contents_dy.append(orig_title)
                                try:
                                    pics_context = paragraphs[1]['pic']['pics']
                                except KeyError:
                                    pics_context = []
                                contents_dy = await add_append_img(contents_dy, await asyncio.gather(*[
                                    asyncio.create_task(download_img(item['url'], f'{filepath}', len=len(pics_context)))
                                    for item in pics_context]))
                    ...
                    if is_twice is True:
                        avatar_path = \
                        (await asyncio.gather(*[asyncio.create_task(download_img(orig_owner_cover, f'{filepath}'))]))[0]
                        if orig_pub_time == '':
                    ...
                    orig_contents, orig_avatar_path, orig_name, orig_Time, orig_type, orig_introduce, orig_emoji_list = await bilibili(
                        orig_url, f'{filepath}orig_', is_twice=True)
                    ...
                    out_path = draw_adaptive_graphic_and_textual(contents, avatar_path=avatar_path,
                                                                 ...
                                                                 contents_dy=orig_contents,
                                                                 orig_avatar_path=orig_avatar_path,
                                                                 orig_name=orig_name, orig_Time=orig_Time,
                                                                 type_software='BiliBili 动态',
                                                                 color_software=(251, 114, 153, 80),
                                                                 output_path_name=f'{dynamic_id}',
                                                                 avatar_json=avatar_json,
                                                                 orig_type_software='转发动态', emoji_list=emoji_list,
                                                                 orig_emoji_list=orig_emoji_list
                                                                 )
                    json_check['pic_path'] = out_path
                    json_check['time'] = pub_time
    ...
    if 'live' in url:
        ...
        owner_name, owner_cover = data_get_url_context['anchor_info']['base_info']['uname'], \
        data_get_url_context['anchor_info']['base_info']['face']
        ...
        if is_twice is not True:
            out_path = draw_adaptive_graphic_and_textual(contents, avatar_path=avatar_path, name=owner_name,
                                                         Time=f'{video_time}', type=12, introduce=introduce,
                                                         filepath=filepath, type_software='BiliBili 直播',
                                                         color_software=(251, 114, 153, 80),
                                                         output_path_name=f'{room_id}', avatar_json=avatar_json)
            json_check['pic_path'] = out_path
    ...
    if is_twice is not True:
        out_path = draw_adaptive_graphic_and_textual(contents, avatar_path=avatar_path, name=owner_name,
                                                     Time=f'{video_time}', type=type, introduce=introduce,
...
async def wb(url, filepath=None):
    ...
    text, status_title, source, region_name, pics, page_info = (weibo_data.get(key, None) for key in
                                                                ['text', 'status_title', 'source', 'region_name',
                                                                 'pics', 'page_info'])
    owner_name, avatar_hd, video_time = weibo_data['user']['screen_name'], weibo_data['user']['avatar_hd'], weibo_data[
        'created_at']
    download_img_funcs = [asyncio.create_task(download_img(avatar_hd, f'{filepath}',
                                                           headers={
                                                                       "Referer": "http://blog.sina.com.cn/"} | COMMON_HEADER))]
    avatar_path = await asyncio.gather(*download_img_funcs)
    ...
    if pics:
        ...
        download_img_funcs = [asyncio.create_task(download_img(f'{item}', f'{filepath}', len=length)) for item in
                              img_check]
        links_path = await asyncio.gather(*download_img_funcs)
    ...
    if page_info:
        ...
        if 'page_pic' in page_info:
            if page_info.get('type') != 'topic' and page_info.get('type') != 'place':
                ...
                download_img_funcs = [asyncio.create_task(download_img(page_pic, f'{filepath}', headers={
                                                                                                            "Referer": "http://blog.sina.com.cn/"} | COMMON_HEADER))]
...
async def xiaohongshu(url, filepath=None):
    ...
    avatar_path = \
    (await asyncio.gather(*[asyncio.create_task(download_img(note_data['user']['avatar'], f'{filepath}'))]))[0]
...
async def Galgame_manshuo(url, filepath=None):
    ...
    for context_check in context:
        ...
        if '登陆后才可以评论获取资源哦～～' in context_check:
            ...
            if hikarinagi_flag == 0:
                links_url = (re.findall(r"https?://[^\s\]\)]+", context_check))[0]
                Title = context_check.replace(" ", "").replace(f"{links_url}", "").replace("[", "").replace("]",
                                                                                                            "").replace(
                    " - Hikarinagi", "").replace("(", "").replace(")", "")
                hikarinagi_flag = 1
...
async def youxi_pil_new_text(filepath=None):
    ...
    async with httpx.AsyncClient() as client:
        ...
        contents = await add_append_img(contents, await asyncio.gather(
            *[asyncio.create_task(download_img(data_src_values[0], f'{filepath}'))]))
...
async def bangumi_PILimg(text=None, img_context=None, filepath=None, proxy=None, type_soft='Bangumi 番剧', name=None,
                         url=None,
...
async def gal_PILimg(text=None, img_context=None, filepath=None, proxy=None, type_soft='Bangumi 番剧', name=None,
                     url=None,
                     type=None, target=None, search_type=None):
    contents = []
    json_check = copy.deepcopy(json_init)
