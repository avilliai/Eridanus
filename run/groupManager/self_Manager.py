...
async def report_to_master(bot,event,config,mes_type):
    if mes_type=="bad_content":
        ...
        r = await aiReplyCore_shadow([{
                                          "text": f"来自用户的反馈: 向昵称为{config.basic_config['master']['name']}的管理员报告id为{event.user_id}，昵称为{event.sender.nickname}的用户反馈了 {event.raw_message}。上报消息中必须保留发送者id即{event.user_id}"}],
