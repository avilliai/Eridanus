...
async def operate_group_push_tasks(bot,event:GroupMessageEvent,config,task_type:str,operation:bool,target_uid:int=None):
    ...
    if task_type=="asmr":
        if operation:
            if event.group_id in config.scheduled_tasks.scheduledTasks_push_groups_ordinary["nightASMR"]["groups"]:
                await bot.send(event,"本群已经订阅过了")
                return
            else:
                config.scheduled_tasks.scheduledTasks_push_groups_ordinary["nightASMR"]["groups"].append(event.group_id)
                config.save_yaml("scheduledTasks_push_groups_ordinary",plugin_name="scheduled_tasks")
                await bot.send(event,"订阅成功")
        else:
            if event.group_id in config.scheduled_tasks.scheduledTasks_push_groups_ordinary["nightASMR"]["groups"]:
                config.scheduled_tasks.scheduledTasks_push_groups_ordinary["nightASMR"]["groups"].remove(event.group_id)
                config.save_yaml("scheduledTasks_push_groups_ordinary",plugin_name="scheduled_tasks")
