...
def main(bot, config):
    ...
    async def startYouridentify(event: GroupMessageEvent):
        ...
        if event.pure_text == "识别" or (event.get("at") and event.get("at")[0]["qq"] == str(bot.id) and event.get(
                "text") is not None and "识别" in event.get("text")[0]):
        ...
        if ((event.get("text") is not None and "识别" in event.get("text")[0]) or (
                event.user_id in image_identify_list)) and event.get('image') is not None:
