...
def main(bot, config):
    ...
    async def check_for_majsoul_personal_info_run(event: GroupMessageEvent):
        ...
        if check_flag in [2, 4, 5]:
            canvas_width = 1400
        else:
            canvas_width = 1200
        majsoul_pil_context = f'{majsoul_json["text"]}\n#”雀魂查询“ 指令菜单：#\n雀魂信息、雀魂记录、雀魂注册\n雀魂四麻查询、雀魂三麻查询、雀魂四麻记录、雀魂三麻记录'
        majsoul_pil_json = await majsoul_PILimg(text=majsoul_pil_context, filepath='data/pictures/cache/',
                                                type_soft=majsoul_json["type"], canvas_width=canvas_width)
