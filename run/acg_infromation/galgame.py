...
def main(bot, config):
    ...
    async def galgame_group_check(event: GroupMessageEvent):
        ...
        if ("galgame推荐" == str(event.pure_text) or "Galgame推荐" == str(event.pure_text) or "gal推荐" == str(
                event.pure_text) or "Gal推荐" == str(event.pure_text)
                or ("随机" in str(event.pure_text) and (
                        "gal" in str(event.pure_text) or "Gal" in str(event.pure_text)))):
            flag = 8
            flag_check_test = 3
