...
async def modelscope_tts(text: str, speaker: str):
    speaker_voice = speakers.get(speaker)
    if not speaker_voice:
        return None
    if speaker_voice.get('speaker'):
        speaker = speaker_voice.get('speaker')
    url = speaker_voice.get('url')
    newurp = speaker_voice.get('newurp')
