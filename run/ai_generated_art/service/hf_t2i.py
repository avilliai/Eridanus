    ...
    try:
        ...
        positive = str(
            args.get('p', default_args.get('p', positives)) if isinstance(args, dict) and isinstance(default_args,
                                                                                                     dict) else args.get(
                'p', positives) if isinstance(args, dict) else default_args.get('p', positives) if isinstance(
                default_args, dict) else positives)
        negative = str(
            args.get('n', default_args.get('n', negatives)) if isinstance(args, dict) and isinstance(default_args,
                                                                                                     dict) else args.get(
                'n', negatives) if isinstance(args, dict) else default_args.get('n', negatives) if isinstance(
                default_args, dict) else negatives)
        positive = (("{}," + positive) if "{}" not in positive else positive).replace("{}", prompt) if isinstance(
            positive, str) else str(prompt)
        positive, _ = await replace_wildcards(positive)
        steps = int(
            args.get('steps', default_args.get('steps', 35)) if isinstance(args, dict) else default_args.get('steps',
                                                                                                             35)) if isinstance(
            default_args, dict) else 35
        ...
        payload = {
            "data": [
                positive,
                negative,
                random.randint(0, 2 ** 32 - 1), width, height, cfg, steps, "Euler a", "Custom", "(None)", False, 0.55,
                1.5, True
            ],
            "event_data": None,
            "fn_index": 5,
