        ...
        try:
            ...
            if config.resource_collector.config["iwara"]["zip_file"]:
                ...
                if config.resource_collector.config["iwara"]["zip_password"]:
                    compress_files_with_pwd(list.get('path'), "data/video/cache", zip_name=zip_name,
                                            password=config.resource_collector.config["iwara"]["zip_password"])
                    await bot.send(event,
                                   Text(f"文件压缩中，密码：{config.resource_collector.config['iwara']['zip_password']}"))
                else:
