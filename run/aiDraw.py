...
async def get_img(processed_message):
    for i in precessed_message:
        if "image" in i or "mface" in i:
            ...
            try:
                ...
                message = await get_img(event_obj.processed_message)
                return message
            except Exception as e:
...
def main(bot,config):
    ...
    async def search_image(event):
        try:
            ...
            if ("ai图检测" in str(event.pure_text) or event.sender.user_id in ai_img_recognize) and await get_img(event.processed_message):
                img_url = await get_img(event.processed_message)
    ...
    async def tagger(event):
        ...
        # 处理图片和重绘命令
        if (str(event.pure_text).startswith("tag") or event.sender.user_id in tag_user) and await get_img(event.processed_message):
            if config.api['ai绘画']['sd审核和反推api'] == "" or config.api['ai绘画']['sd审核和反推api'] == None:
                await bot.send(event, "未配置审核和反推api")
                return
    ...
    async def sdreDrawRun(event):
        ...
        # 处理图片和重绘命令
        if (str(event.pure_text).startswith("重绘") or event.sender.user_id in UserGet) and await get_img(event.processed_message):
            if (str(event.pure_text).startswith("重绘")) and await get_img(event.processed_message):
                prompt = str(event.pure_text).replace("重绘", "").strip()
                UserGet[event.sender.user_id] = [prompt]
    ...
    async def AiSdDraw(event):
        ...
        if str(event.pure_text) == "ckpt" and config.settings["ai绘画"]["sd画图"]:  # 获取lora列表
        ...
        if str(event.pure_text).startswith("ckpt2 ") and config.settings["ai绘画"]["sd画图"]:
        ...
        if str(event.pure_text) == "scheduler" and config.settings["ai绘画"]["sd画图"]:
        ...
        if str(event.pure_text) == "interrupt" and config.settings["ai绘画"]["sd画图"] and event.user_id == config.basic_config["master"]["id"]:
            global turn
            try:
    ...
    async def n4reDrawRun(event):
        ...
        # 处理图片和重绘命令
        if (str(event.pure_text).startswith("n4re") or event.sender.user_id in n4re) and await get_img(event.processed_message):
            if (str(event.pure_text).startswith("n4re")) and await get_img(event.processed_message):
                prompt = str(event.pure_text).replace("n4re", "").strip()
                n4re[event.sender.user_id] = [prompt]
    ...
    async def n3reDrawRun(event):
        ...
        # 处理图片和重绘命令
        if (str(event.pure_text).startswith("n3re") or event.sender.user_id in n3re) and await get_img(event.processed_message):
            if (str(event.pure_text).startswith("n3re")) and await get_img(event.processed_message):
                prompt = str(event.pure_text).replace("n3re", "").strip()
                n3re[event.sender.user_id] = [prompt]
    ...
    async def sdmaskDrawRun(event):
        ...
        if event.sender.user_id in UserGetm and await get_img(event.processed_message) and event.sender.user_id in mask:
            path = f"data/pictures/cache/{random_str()}.png"
            prompts = UserGetm[event.sender.user_id]  # 直接使用字符串
            mask_url = await get_img(event.processed_message)
        ...
        if (str(event.pure_text).startswith("局部重绘") or event.sender.user_id in UserGetm) and await get_img(event.processed_message):
            if (str(event.pure_text).startswith("局部重绘 ")) and await get_img(event.processed_message):
